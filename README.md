# 🧮 Simple Calculator (Vishal Edition)

A sleek, playful calculator built with **HTML, CSS, and JavaScript** — but with a twist.  
This isn’t just math… it’s math with **animations and easter eggs**

---

## ✨ Features

- **Basic Operations**: Addition, subtraction, multiplication, division.
- **Responsive Display**: Numbers slide, fade, or pop into view when typed.
  
---

## 🎨 Cool Animations

- **Button Bounce**: Buttons grow and bounce when clicked.
- **Display Effects**: Numbers can fade in, slide in, or glow.
- **Error Feedback**: Red flash + shake when input is invalid.
- **Special Spin**: Division by zero triggers a spinning `Infinity`.

---

## 🥳 Hidden Surprise

There’s a secret waiting for you.  
👉 Type `7092007` into the calculator and press `=`.  

---

Inspired by my Fundementals of web application Development: professor Dr. Selva Sachein at Saveetha Engineering college
